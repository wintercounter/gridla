...
/**
 * Vue adapter contract against `examples/adapters/vue/`: a dashboard bound with
 * `v-model:layout`, a group item hosting a nested provider inside a transfer
 * scope, built-in resize handles, a status line showing the last commit, and
 * JSON readouts of both layouts.
 */
...
test.describe('adapter vue', () => {
  ...
  test('drag commits through v-model:layout and reports the strategy', async ({ page }) => {
    ...
    await expect(page.locator('#status')).toHaveText(/last commit: outer · move · [a-z-]+ · chart/)
    // The parent's state (rendered through the JSON readout) matches the DOM.
  ...
  test('resize via the east handle grows the item and updates the readout', async ({ page }) => {

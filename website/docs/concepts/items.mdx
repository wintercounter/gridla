...
- `normalizeItem(item, canvas)` rounds, enforces `min` and `max`, and fits the item inside the padded area. It shrinks before it moves: the position only shifts when the minimum size would not fit otherwise. Scrollable canvases do not clamp `y` or `h`.
...
normalizeLayout(raw).items[0] // { x: 200, y: 100, w: 200, h: 200, minW: 100, minH: 100, ... }
```

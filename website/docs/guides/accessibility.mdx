...
Items expose `data-gridla-active`, `data-gridla-selected`, `data-gridla-shifted`, and `data-gridla-transferring` attributes. Use them to draw states that do not rely on color alone: a dashed border for shifted siblings, an outline for selection, and a visible focus ring (the tokens use a two-ring `box-shadow` that reads on both themes).
